...
const CustomerLayout = () => {
  ...
  const iconMap = {
    ...
    "book-shipment": <Package className="w-3.5 h-3.5" />,
    "my-bookings": <BookOpen className="w-3.5 h-3.5" />,
    payments: <CreditCard className="w-3.5 h-3.5" />,
    "track-shipment": <MapPin className="w-3.5 h-3.5" />,
        ...
        <header className="sticky top-0 z-30 flex items-center justify-between gap-2 sm:gap-4 px-3 sm:px-6 py-2.5 sm:py-3 bg-white/90 dark:bg-gray-900/90 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 shadow-sm">
          ...
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <div className="flex items-center gap-1 text-xs sm:text-sm text-gray-500 dark:text-gray-400 overflow-x-auto no-scrollbar">
              <span className="capitalize text-[10px] sm:text-xs font-semibold px-2 py-0.5 rounded bg-purple-100 dark:bg-purple-950/60 text-purple-700 dark:text-purple-400 whitespace-nowrap">
                Customer
              </span>
              {pathnames.slice(0, 3).map((value, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Link
                      to={to}
                      className={`hover:text-purple-600 dark:hover:text-purple-400 flex items-center gap-1 sm:gap-1.5 transition-colors whitespace-nowrap ${
                        last
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
            ...
            >
              ...
              <div className="w-8 h-8 sm:w-9 sm:h-9 bg-purple-600 rounded-full flex items-center justify-center text-white font-bold ring-2 ring-purple-500/20 shadow-inner text-sm overflow-hidden shrink-0">
                {user?.profileImage ? (
                  <img
        ...
        <main className="flex-1 overflow-y-auto overflow-x-hidden p-3 sm:p-6 bg-gray-50 dark:bg-gray-950 flex flex-col justify-between">
          ...
          <footer className="mt-8 pt-4 border-t border-gray-200 dark:border-gray-800/80 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-500 dark:text-slate-400">
            ...
            <div className="flex items-center gap-4 text-xs">
              <Link to="/about" className="hover:text-purple-600 dark:hover:text-purple-400 transition">
                About NTMS
              </Link>
              <Link to="/contact" className="hover:text-purple-600 dark:hover:text-purple-400 transition">
                Help & Support
              </Link>
              <Link to="/customer/payments" className="hover:text-purple-600 dark:hover:text-purple-400 transition">
                Payment History
              </Link>
              <Link to="/customer/track-shipment" className="hover:text-purple-600 dark:hover:text-purple-400 transition">
                Live Tracking
              </Link>
            </div>
          </footer>
